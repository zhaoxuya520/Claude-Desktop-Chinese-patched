...
面向 Windows 版 Claude Desktop 的中文化项目（已在 `1.15962.x` 上验证）。

> 启动器用 `Get-AppxPackage` 自动识别本机已安装的最新版 Claude，**不写死版本号**；官方升级后一般无需改动，官方中文也会随安装版本自动跟随。
...
它基于原始项目 [Po1nt9/Claude-Desktop-Chinese](https://github.com/Po1nt9/Claude-Desktop-Chinese) 继续演进，并在其基础上新增了“正版客户端代理汉化”方案（最初针对 `1.6259.1.x`，现已适配 `1.15962.x`）。
...
- `resources/frontend-dynamic-zh-CN.json`
- `run-official-zh-proxy.bat`
- 正版 `Claude.exe` 通过 **AUMID 激活 + 本地代理**启动、并在运行时读取官方中文的整套链路
...
1. 通过 **AUMID 应用激活**（`shell:AppsFolder\<PackageFamilyName>!Claude`）启动正版 Claude，并把 `--proxy-server / --ignore-certificate-errors / --lang=zh-CN` 透传进去。
2. 让它通过本地代理访问 `claude.ai`。
3. 拦截语言资源请求（`/i18n/*.json`、`/i18n/dynamic/*` 等）。
4. 把远端默认英文语言包替换为中文——**优先使用本机已安装 Claude 自带的官方中文**（运行时读取，约 95% 覆盖），辅以仓库内兜底与少量手译。
5. 再补一层页面级文本替换，处理少量硬编码英文。

> 注：直接用完整路径启动 `WindowsApps\...\Claude.exe` 在新版上会**闪退**（进程在主进程日志初始化前就退出），因此必须走 AUMID 激活——启动器已自动处理。
...
1. 关闭现有 Claude Desktop（只关 `WindowsApps` 里的 Desktop，**不会误关同名的 Claude Code**）与旧代理进程
2. 启动本地中文代理（运行时读取本机 Claude 自带的官方中文）
3. 清理安全缓存（MSIX 真实容器路径）
4. 通过 **AUMID 激活**、以 `--proxy-server=http://127.0.0.1:8877 --ignore-certificate-errors --lang=zh-CN` 启动官方 Claude
...
- `resources/frontend-zh-CN.json`
  主前端中文语言包（**兜底**；运行时会优先合并本机 Claude 自带的官方中文，仓库不打包官方串）

- `resources/frontend-dynamic-zh-CN.json`
  动态端点（模型 / 思考选择器）补充中文；思考等级名（`Max/High/Low` 等）按习惯保留英文，仅翻译解释
...
- 当前主要验证过 `1.15962.x`（启动器自动适配本机已安装的最新版本，不写死版本号）
- 官方中文随本机已安装版本**自动跟随**，仓库内不打包、不分发官方翻译串
- Claude 官方一旦调整 `claude.ai` 的 i18n 端点或落地路由，代理的拦截规则可能需要小幅适配
